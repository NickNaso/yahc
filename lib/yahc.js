...
/**
 * @description This function returns a collection of status code error based
 * on type of response (INFORMATIONAL - SUCCESS - REDIRECTION - CLIENT_ERROR
 * - SERVER_ERROR).
 * @param {String} responseType String represents response type.
 * @return {Array} Array or collection of status codes relative to specified
 * response type.
 * @private
 */
...
/**
 * @description This function processes the server response
 * @param {object} response Represents the response object coming from the
 * server.
...
/**
 * @description This function checks if the file resource is expressed as
 * string, Buffer or Readable stream and returns true or false respectively.
 * @param {Buffer | Readable} resource Represents the resource file to send to 
 * the server.
 * @return {boolean} boolean Returns true if the resource is a Buffer or a
 * Readable stream.
 * @private
 */
function _checkResourceType(resource) {
    return Buffer.isBuffer(resource) || resource instanceof Readable;
}

/**
 * @description This function checks if the resource object is expressed in a 
 * good way.
 * @param {object} resource Represents the resource file to send to the server
 * @return {boolean} boolean Returns true if the resource object has the 
 * following form {name: "string", file:"Buffer | Readable" }.
 * @private
 */
function _checkValidObjectResource(resource) {
    let checkObjForm = _.isObject(resource) && (!_.isEmpty(resource)) && 
        _.has(resource, 'name') && _.has(resource, 'file');
    if (!checkObjForm) {return false;}    
    let name = resource.name;                  
    let checkNameField = typeof name !== "string" || name.length === 0 || 
        name.trim().length === 0; 
    if (checkNameField) {return false;}    
    let file = resource.file;
    let checkFileField =  _checkResourceType(file);
    if (!checkFileField) {return false;}
    return true;                           
}
...
module.exports = {
    ...
    /**
     * @description This property represents the time, expressed in
     * milliseconds, in which the client has to receive the response otherwise
     * will throw an error.
     * @public
     */
    TIMEOUT: 15000,
    ...
    STATUS_CODE: {
        ...
        // which the server is aware that it has erred or it is incapable to 
        // perform the request. Except when responding to a HEAD request, the
    ...
    /**
     * @description This method returns an array or a collection of all 
     * INFORMATIONAL status codes.
     * @return {Array} Array or collection of all status codes that could be 
    ...
    /**
     * @description This method returns an array or a collection of all SUCCESS 
     * status codes.
     * @return {Array} Array or collection of all status codes that could be 
     * grouped as SUCCESS.
     * @public
     ...
     * of all query parameters in the request.
     * @param {string} [options.encType = application/x-www-form-urlencoded] 
     * This parameter is a string representing the encoding type for the
     * request.
     ...
     * @return {object} This method returns an object that represents the server
            ...
            if (encType === this.ENC_TYPES.X_WWW_FORM_URLENCODED && isJson) {
                ...
                if (files.length === 0) {
                    throw new RestClientError("Sorry rest client encountered" +
                        " an error in executing request. If you set encType" +
                        " to multipart/form-data the files parameter in POST" +
                        " request cannot be empty." );
                }
                let filesToUpload = [];
                for (let resource of files) {
                    if (_checkResourceType(resource)) {
                        filesToUpload.push(resource);
                    } else if (_checkValidObjectResource(resource)) {    
                        body[resource.name] = resource.file;
                    } else {
                        throw new RestClientError("Sorry rest client" +
                            " encountered an error in executing POST request." + 
                            " You inserted not valid value type in files" +
                            " parameter.");
                    }
                }
        ...
        try {
            ...
            if (encType === this.ENC_TYPES.X_WWW_FORM_URLENCODED && isJson) {
                ...
                if (files.length === 0) {
                    throw new RestClientError("Sorry rest client encountered" +
                        " an error in executing request. If you set encType" +
                        " to multipart/form-data the files parameters in PUT" +
                        " request cannot be empty." );
                }
                let filesToUpload = [];
                for (let resource of files) {
                    if (_checkResourceType(resource)) {
                        ...
                        throw new RestClientError("Sorry rest client" +
                            " encountered an error in executing PUT request." + 
                            " You inserted not valid value type in files" +
                            " parameter.");
                    }
                }

...
function _getStatusCodeByResponseType (responseType) {
    return _.map(_.values(responseType), (STATUS_CODE) => {return STATUS_CODE.CODE;});
}
...
module.exports = {
    ...
    /**
     * @description This object represents the sets of HTTP Status code information found at
     * @see {@link http://www.ietf.org/assignments/http-status-codes/http-status-codes.xml|ietf.org}
     * @property {object} INFORMATIONAL This object represents a sets of key - value regard
     * status code indicates a provisional response, consisting only of the Status-Line
     * and optional headers, and is terminated by an empty line.
     * @property {object} SUCCESS  This object represents a sets of key - value regard
     * status code indicates that the client's request was successfully received, understood,
     * and accepted.
     * @property {object} REDIRECTION This object represents a sets of key - value
     * regard status code and indicates that further action needs to be taken by
     * the user agent in order to fulfill the request.
     * @property {object} CLIENT_ERROR This object represents a sets of key - value
     * regard status code and indicates the cases in which the client seems to
     ...
     * entity to the user.
     * @property {object} SERVER_ERROR This object represents a sets of key - value
     * regard status code and indicates cases in which the server is aware that
    ...
    /**
     * @description This method return an array or collection of all INFORMATIONAL
     * status code.
     * @return {Array} Array or collection of all status code that could be grouped
     * as INFORMATIONAL.
     * @public
     */
    ...
    /**
     * @description This method return an array or collection of all SUCCESS status
     * code.
     * @return {Array} Array or collection of all status code that could be grouped
     * as SUCCESS.
     * @public
    ...
    /**
     * @description This method return an array or collection of all CLIENT_ERROR
     * status code.
     * @return {Array} Array or collection of all status code that could be grouped
     * as CLIENT_ERROR.
     * @public
    ...
    /**
     * @description This method return an array or collection of all SERVER_ERROR
     * status code.
     * @return {Array} Array or collection of all status code that could be grouped
     * as SERVER_ERROR.
     * @public
     ...
     */
    * GET({url = null, headers = null, qs = null, encType = this.ENC_TYPES.X_WWW_FORM_URLENCODED, isJson = true} = {}) {
        if (!url) {
            throw new RestClientError("Sorry rest client encountered an error in" +
                " executing request. Url parameter in GET request cannot be null.");
        }
        try {
            ...
            if (encType === this.ENC_TYPES.X_WWW_FORM_URLENCODED) {
                ...
                let statusCodes = [... this.getInformationalStatusCode(), ... this.getSuccessStatusCode()];
                return _handleHTTPResponse(response, statusCodes);
            }
            else {
                throw new RestClientError("Sorry rest client encountered an error" +
                    " in executing request. Permitted data types in GET request are:" +
     ...
     * response about the requested resource.
     */
    * POST({url = null, headers = null, qs = null, body = null, encType = this.ENC_TYPES.X_WWW_FORM_URLENCODED, isJson = true, files =[]} = {}) {
        if (!url) {
            throw new RestClientError("Sorry rest client encountered an error in" +
                " executing request. Url parameter in POST request cannot be null.");
        }
        try {
            // Init request options
            let requestOpts = {
            ...
            if (encType === this.ENC_TYPES.X_WWW_FORM_URLENCODED) {
                ...
                if (files.length === 0) {
                    throw new RestClientError("Sorry rest client encountered" +
                        " an error in executing request. The excType parameter has" +
                        " been setted to: " + encType + " but there are not files" +
                        " to upload.");
                }
                body = body || {};
                ...
                throw new RestClientError("Sorry rest client encountered an error" +
                    " in executing request. Permitted data types in POST request are:" +
     ...
     * response about the requested resource.
     */
    * PUT({url = null, headers = null, qs = null, body = null, encType = this.ENC_TYPES.X_WWW_FORM_URLENCODED, isJson = true, files =[]} = {}) {
        if (!url) {
            throw new RestClientError("Sorry rest client encountered an error in" +
                " executing request. Url parameter in POST request cannot be null.");
        }
        try {
            ...
            let statusCodes = [... this.getInformationalStatusCode(), ... this.getSuccessStatusCode()];
            if (encType === this.ENC_TYPES.X_WWW_FORM_URLENCODED) {
                console.log("Data type => ", this.ENC_TYPES.X_WWW_FORM_URLENCODED);
                requestOpts.body = body || {};
                response = yield request(requestOpts);
            } else if (encType === this.ENC_TYPES.MULTIPART_FORM_DATA) {
                if (files.length === 0) {
                ...
                throw new RestClientError("Sorry rest client encountered an error" +
                    " in executing request. Permitted data types in PUT request are:" +
                    " application/x-www-form-urlencoded - multipart/form-data");
            }
            return _handleHTTPResponse(response, statusCodes);
        } catch (e) {
     ...
     * response about the requested resource.
     */
    * DELETE({url = null, headers = null, qs = null, body = null, encType = this.ENC_TYPES.X_WWW_FORM_URLENCODED, isJson = true} = {}) {
        if (!url) {
            throw new RestClientError("Sorry rest client encountered an error in" +
                " executing request. Url parameter in DELETE request cannot be null.");
        }
        try {
            // Init request options
            let requestOpts = {
            ...
            if (encType === this.ENC_TYPES.X_WWW_FORM_URLENCODED) {
                ...
                throw new RestClientError("Sorry rest client encountered an error" +
                    " in executing request. Permitted data types in DELETE request are:" +
                    " application/x-www-form-urlencoded.");
            }
            return _handleHTTPResponse(response, statusCodes);
        } catch (e) {
